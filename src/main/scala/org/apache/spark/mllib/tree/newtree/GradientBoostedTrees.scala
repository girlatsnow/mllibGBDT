...
package org.apache.spark.mllib.tree.newtree
...
import org.apache.spark.mllib.tree.newtree
...
class GradientBoostedTrees @Since("1.2.0") (private val boostingStrategy: BoostingStrategy)
  ...
  /**
    * Java-friendly API for [[GradientBoostedTrees!#run]].
    */
  @Since("1.2.0")
  ...
  /**
    * Java-friendly API for [[GradientBoostedTrees!#runWithValidation]].
    */
  @Since("1.4.0")
  def runWithValidation(
...
object GradientBoostedTrees extends Logging {
  ...
  private def boost(
    ...
    val firstTreeModel = new newtree.DecisionTree(treeStrategy).run(input)
    ...
    println("error of gbt = " + predError.values.mean())
    ...
    while (m < numIterations && !doneLearning) {
      ...
      val model = new newtree.DecisionTree(treeStrategy).run(data)
