...
private class RandomForest (
  ...
  def run(input: RDD[LabeledPoint]): RandomForestModel = {
    ...
    val (splits, bins) = newtree.DecisionTree.findSplitsBins(retaggedInput, metadata)
    ...
    while (nodeQueue.nonEmpty) {
      ...
      println(s"numNodes = ${nodesForGroup.values.map(_.length).sum}")

      // Choose node splits, and enqueue new nodes as needed.
      timer.start("findBestSplits")
      newtree.DecisionTree.findBestSplits(baggedInput, metadata, topNodes, nodesForGroup,
